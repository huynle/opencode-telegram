...
export async function createIntegratedApp(config: AppConfig): Promise<IntegratedApp> {
  ...
  // Create Telegram send callback for stream handler
  const sendCallback: TelegramSendCallback = async (chatId, topicId, text, options) => {
    try {
      ...
      console.error("[Integration] Failed to send Telegram message:", error)
      throw error
    }
  }
    ...
    switch (event.type) {
      case "instance:ready": {
        ...
        if (instance) {
          ...
          streamHandler.registerSession(sessionId, config.telegram.chatId, topicId)
  ...
  async function routeMessageToInstance(
    ...
    // Get topic name from store or use default
    const mapping = topicStore.getMapping(chatId, effectiveTopicId)
    const topicName = mapping?.topicName || `topic-${effectiveTopicId}`
    const workDir = `${config.project.basePath}/${topicName}`

    // Ensure directory exists
    if (config.project.autoCreateDirs) {
      try {
